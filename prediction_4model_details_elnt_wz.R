# library(varbvs, lib = "/gpfs/loomis/scratch60/fas/zhao/wz284/R")
# library(glmnet, lib = "/gpfs/loomis/scratch60/fas/zhao/wz284/R")

args <- commandArgs()
# args[6]: chr; args[7]: gene;
...
annot.dir <- paste0("/ysm-gpfs/pi/zhao/wl382/snpPred_epi/Annotation/data/annotation/",chr,"/",gene,"/")
files = list.files(annot.dir)


gene_dir = paste0("/ysm-gpfs/pi/zhao/from_louise/jw2372/GTEx/cis_snp_by_gene/",chr,"/",gene_snp,"/")
expr_dir = paste0("/ysm-gpfs/pi/zhao/from_louise/yh367/GTEX/adjusted_expr1/",chr,"/",gene_exp,"/")
snp_info_file = paste0(gene_dir,gene_snp,".mach.info")
if(!file.exists(snp_info_file)){
  next()
}
snp_info = read.table(snp_info_file, header = T, stringsAsFactors = F,fill=T)
result_dir = paste0("/ysm-gpfs/scratch60/wz284/snpPred_epi/Annotation/data/model/",chr,"/",gene,"/")
  ...
  annot_file = paste0("/ysm-gpfs/pi/zhao/wl382/snpPred_epi/Annotation/data/annotation/",chr,"/",gene,"/annotation_",tissue,"_test_1.txt")
  if(!file.exists(annot_file)){
    annot_file = paste0("/ysm-gpfs/pi/zhao/wl382/snpPred_epi/Annotation/data/annotation/",chr,"/",gene,"/annotation_",tissue,".txt")
    if(!file.exists(annot_file)){
 ...
 #   next()
  ...
  # gen_file = paste0(gene_dir,"Muscle_Skeletal_gen_AD_interAnnot.txt")
 ...
 # x.array[,i,1] = gen_sort_1[,i]
  ...
  save(x.array,y.array,annotat.array,ind.2,
    betas.elnt,betas.elnt.annot,betas.vb,betas.vb.annot,
    top.vars.vb,top.vars.vb.annot,mde.varbvs,mde.varbvs.annot,
    file=paste0(result_dir,gene,"_",tissue,"_4model_CenteredXOriginalY.Rdata"))  
#}
#save(x.array,y.array,annotat.array,ind.2,
